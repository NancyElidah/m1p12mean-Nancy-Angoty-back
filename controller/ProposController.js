...
class ProposController {
  ...
  createPropos = async (req, res) => {
    try {
      ...
      //   if (user.statut == 0) {
          ...
          res.status(201).send({propos: propos });
        // } else {
  ...
  getAll = async (req, res) => {
    try {
      const propos = await this.proposService.findAll(); 
      res.status(200).json({propos});
    } catch (error) {
    ...
    try {
      ...
      //   const user = await this.tool.get_user_online(req);
        // if (user.statut == 0) {
          ...
          res.status(200).send({message: "Propos supprimé." });
        // } else {
        //   res.status(403).send({ droit: false, message: "Utilisateur non autorisé." });
        // }
